...
game_score <- rbind(
  game_score,
  c(bears = 37, opponent = 17),
  c(bears =  9, opponent = 17)
)
print(game_score)
```
...
for(i in seq_len(nrow(game_score))) {
  if(game_score[i, "bears"] > game_score[i, "opponent"]) cat("Win\n") else cat("Loss\n")
}
```
...
```{r}
first_win_idx <- match(1, game_score[, "win"])
print(first_win_idx-1)
```
...
```{r}
number_of_factors <- function(n) sum(n %% (1:n) == 0)
...
longest_words <- function(words) {
  lengths <- nchar(words)
  words[lengths == max(lengths)]
}

print(longest_words(fruits))
```

## Question 6 (4 points)

Write a function that checks if a value is in a vector. The function should take both a value and vector as inputs and return either TRUE or FALSE.
...
```{r}
in_vector <- function(value, vec) value %in% vec
value <- 2
check <- seq(1, 15, 3)
result <- in_vector(value, check)
if (result) cat(sprintf("%d is in the vector", value)) else cat(sprintf("%d is not in the vector.", value))
...
```{r}
replace_negatives <- function(n) {
  n[n < 0] <- NA
  n
}
new_x <- replace_negatives(x)
print(new_x)
...
```{r}
even_and_odd <- function(m) {
  ifelse(m %% 2 == 0, 1, 0)
}
new_y <- even_and_odd(x)
print(new_y)
```
